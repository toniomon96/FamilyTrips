    ...
    await runStep(`browser.generated-manage.${label}`, async () => {
      await withCheckedPage(browser, `generated-manage-${label}`, viewport, async (page) => {
        ...
        if (label === 'mobile') {
          ...
          const golfCard = page.locator('article').filter({ hasText: 'Golf' }).first()
          await golfCard.getByText('Quick edit').click()
          await golfCard.getByLabel('Status').selectOption('confirmed')
          await golfCard.getByLabel('Next step').fill('Tee time confirmed by browser UAT.')
          await golfCard.getByRole('button', { name: 'Save live' }).click()
          await expect(page.getByText(/Saved version/i)).toBeVisible({ timeout: 45000 })
          await page.getByLabel('Workspace section').selectOption('mustDos')
          const loversBeachCard = page.locator('article').filter({ hasText: 'Lovers Beach' }).first()
          await loversBeachCard.getByText('Quick edit').click()
          await loversBeachCard.getByLabel('Time window').fill('Late morning')
          await loversBeachCard.getByLabel('Next step').fill('Confirm boat or water taxi with concierge.')
          await loversBeachCard.getByRole('button', { name: 'Save live' }).click()
          await expect(page.getByText(/Saved version/i)).toBeVisible({ timeout: 45000 })
